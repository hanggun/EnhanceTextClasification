...
import time
from config import r8_config as config
import random
...
def plot_features(feature):
    feature = np.concatenate([x.detach().cpu().numpy() for x in feature])
    tsne_vec = TSNE(n_components=2, learning_rate='auto', init='random', perplexity=3).fit_transform(feature)
    colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', 'palevioletred'] * 2
    color_org = ['#0000b3','#006400','#8b0000','#008b8b','#4b0082','#b35c00','#666666','lightpink']
    colors = color_org + colors
    plt.scatter(tsne_vec[:, 0], tsne_vec[:, 1], c=colors)
    plt.show()
...
def test():
    ...
    if config.aug:
        model_save_dir = config.model_save_dir.split('.')
        model.load_state_dict(torch.load(model_save_dir[0] + '.' + model_save_dir[1]))
    else:
        model.load_state_dict(torch.load(config.model_save_dir))
    model.eval()
