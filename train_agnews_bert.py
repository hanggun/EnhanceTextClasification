...
import time
from config import agnews_config as config
import random
...
def load_agnews_data(filename, label_num_per_class=None):
    """加载数据
...
train_data = load_agnews_data(config.train_data_path, label_num_per_class=2500)
np.random.shuffle(train_data)
valid_data = load_agnews_data(config.dev_data_path)
np.random.shuffle(valid_data)
test_data = load_agnews_data(config.test_data_path)
...
def mask_text(input_ids, mask_probability=0.15):
    ...
    for i in range(len(text)):
        ...
        if rand < mask_probability:
            if '##' not in text[i] and text[i] != tokenizer.mask_token:
                text[i] = tokenizer.mask_token
                j = i+1
                while j < len(text) and '##' in text[j]:
