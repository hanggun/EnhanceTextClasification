...
import time
from config import imdb_config as config
import random
...
def load_imdb_data(filename, label_num_per_class=None, is_clean=None):
    """加载数据
...
train_data = load_imdb_data(config.train_data_path, is_clean=True)
np.random.shuffle(train_data)
valid_data = load_imdb_data(config.dev_data_path, is_clean=True)
np.random.shuffle(valid_data)
test_data = load_imdb_data(config.test_data_path, is_clean=True)
