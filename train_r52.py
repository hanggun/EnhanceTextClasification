...
import time
from config import r52_config as config
import random
...
def plot_features(feature):
    feature = np.concatenate([x.cpu().numpy() for x in feature])
    tsne_vec = TSNE(n_components=2, learning_rate='auto', init='random', perplexity=3).fit_transform(feature)
    colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k', '#ffffff'] * 2
    color_org = ['#0000b3','#006400','#8b0000','#008b8b','#4b0082','#b35c00','#666666','#cccccc']
...
def test():
    ...
    with torch.no_grad():
        for batch_ind, batch in pbar:
            ...
            batch = [x.cuda() for x in batch[1:]]
            x, x1, x2, x3, mask, labels = batch
            loss, logits, feature = model(x, x1, x2, mask=mask)
            true_num, current_length = calculate_acc(logits, labels)
            total_true += true_num
            total_length += current_length
            avg_acc = total_true / total_length
